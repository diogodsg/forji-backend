...
interface ProfessionalJourneySummaryProps {
  className?: string;
}
...
export function ProfessionalJourneySummary({
  className = "",
}: ProfessionalJourneySummaryProps) {
  const { profile } = usePlayerProfile();

  // Métricas baseadas no perfil de gamificação e PDI combinados
  const metrics: SummaryMetric[] = [
    ...
    },
    {
      ...
      trend: (profile?.badges?.length || 0) > 0 ? "up" : "neutral",
      trendValue:
  ...
  // Calcular progresso baseado no nível do jogador
  const progressPercentage = profile
    ? Math.min((profile.level / 100) * 100, 95)
    : 0;

...
export function EditCycleModal({ cycle, onClose, onUpdate }: EditCycleModalProps) {
  ...
  const getStatusOptions = () => {
    ...
    switch (cycle.status) {
      case 'planned':
        return options.filter(opt => ['planned', 'active'].includes(opt.value));
      case 'active':
        return options.filter(opt => ['active', 'paused', 'completed'].includes(opt.value));
      case 'paused':
        return options.filter(opt => ['paused', 'active', 'completed'].includes(opt.value));
      case 'completed':
        return options.filter(opt => ['completed', 'active'].includes(opt.value));
      default:
        return options;
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg">
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <div>
            <h2 className="text-base font-semibold text-gray-900">Editar Ciclo</h2>
            <div className="flex items-center gap-3 mt-0.5 text-xs text-gray-400">
              <span><kbd className="px-1 py-0.5 bg-gray-100 rounded text-xs">Esc</kbd> fechar</span>
              <span><kbd className="px-1 py-0.5 bg-gray-100 rounded text-xs">Ctrl+Enter</kbd> salvar</span>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-3">
          ...
          <div className="bg-gray-50 p-2 rounded">
            <h4 className="text-xs font-medium text-gray-700 mb-1">Dados do PDI neste ciclo</h4>

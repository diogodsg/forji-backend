...
export function ActiveCycleBadge({ cycle }: ActiveCycleBadgeProps) {
  ...
  return (
    <div className={`inline-flex items-center gap-3 pl-3 pr-4 py-1 rounded-md text-xs font-medium border ${statusColor[cycle.status] || 'bg-gray-100 text-gray-700 border-gray-300'}`}> 
      {icon}
      <span className="truncate max-w-[140px] font-semibold" title={cycle.title}>{cycle.title}</span>
      <span className="opacity-70">{statusLabel[cycle.status] || cycle.status}</span>
      <span className="opacity-60 whitespace-nowrap">{fmt(cycle.startDate)} → {fmt(cycle.endDate)}</span>
      <span className="flex items-center gap-1 ml-2">
        <span className="w-24 h-1.5 bg-white/40 rounded overflow-hidden">
          <span className="block h-full bg-current transition-all" style={{ width: `${pct}%` }} />
        </span>

...
export function StateViewer() {
  const { managerState, storeState, metrics, openModals } = useCycleDebugState();
  ...
  const debugInfo = {
    ...
    currentCycle: {
      ...
      status: managerState.cycle?.status,
      progressPercentage: managerState.cycle?.progressPercentage,
      daysRemaining: managerState.cycle?.daysRemaining,
      xpEarned: managerState.cycle?.xpEarned,
    },
  ...
  const goalSummary = managerState.goals?.map((goal, index) => ({
    index: index + 1,
    id: goal.id,
    title: goal.title.substring(0, 25) + (goal.title.length > 25 ? '...' : ''),
    type: goal.type,
    progress:
      goal.type === "quantity"
        ? `${goal.currentNumber || 0}/${goal.targetNumber || 0}`
        : goal.type === "deadline"
        ? goal.completed
          ? "✅ Done"
          : "⏳ Pending"
  ...
  return (
    <div className="space-y-3">
      ...
      {goalSummary.length > 0 && (
        <div>
          <h4 className="font-medium text-gray-700 mb-2">🎯 Goals Summary ({goalSummary.length})</h4>

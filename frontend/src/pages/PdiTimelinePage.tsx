...
import { generateMockTimelineData } from '../features/pdi/hooks/usePdiTimeline';
...
export function PdiTimelinePage() {
  ...
  const mockCycles = generateMockTimelineData();
  const { cycles, stats, loading, error } = {
    cycles: mockCycles,
    stats: {
      totalCycles: mockCycles.length,
      completedCycles: mockCycles.filter(c => c.status === 'completed').length,
      totalKRs: mockCycles.reduce((sum, c) => sum + c.totalKRs, 0),
      totalKRsAchieved: mockCycles.reduce((sum, c) => sum + c.achievedKRs, 0),
      averageCompletionRate: mockCycles.length > 0 
        ? mockCycles.reduce((sum, c) => sum + (c.completionPercentage || 0), 0) / mockCycles.length 
        : 0,
      totalBadgesEarned: mockCycles.reduce((sum, c) => sum + (c.badges?.length || 0), 0),
      currentStreak: 2,
      competenciesImproved: ['Liderança Técnica', 'Comunicação', 'Arquitetura de Software'],
    },
    loading: false,
    error: null,
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="rounded-2xl border border-surface-300 bg-white shadow-sm p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-600">Taxa Média de Conclusão</div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-surface-300 bg-white shadow-sm p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-600">Competências Desenvolvidas</div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-surface-300 bg-white shadow-sm p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-2xl font-bold text-gray-900">
                    {stats.totalKRs > 0 ? Math.round((stats.totalKRsAchieved / stats.totalKRs) * 100) : 0}%
                  </div>
        ...
        {viewMode === 'summary' && (
          <div className="mt-8 max-w-4xl mx-auto">
            <div className="rounded-2xl border border-surface-300 bg-white shadow-sm p-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Resumo de Desenvolvimento</h2>
              
              {/* Key Achievements */}
              <div className="mb-8">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Principais Conquistas</h3>
                <div className="space-y-3">
                  {cycles
                    .filter(c => c.achievements && c.achievements.length > 0)
                    .flatMap(c => c.achievements?.slice(0, 2) || [])
              ...
              <div className="mb-8">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Evolução de Competências</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {stats.competenciesImproved.map((competency, index) => (
                    <div key={index} className="p-4 border border-surface-300 rounded-lg">
                      <h4 className="font-medium text-gray-900 mb-2">{competency}</h4>
              ...
              {/* Recent Feedback */}
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Feedback Recente</h3>
                <div className="space-y-4">
                  {cycles
                    .filter(c => c.feedback && c.feedback.length > 0)
                    .flatMap(c => c.feedback?.slice(0, 1) || [])
                    .map((feedback) => (
                      <div key={feedback.id} className="p-4 border border-surface-300 rounded-lg bg-surface-50">
